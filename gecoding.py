...
def geocode_offical(gmap_client):
    """
    Geocode the traffic-related Weibo posted by official account
    :param gmap_client: the Google map client
    """
    # load the traffic weibo data posted from official traffic accounts
    official_traffic_data = pd.read_excel(os.path.join(weibo_data_path, '1980308627_final.xlsx'),
                                          index_col=0).reset_index(drop=True)
    geocode_result = []
    print('Geocoding the traffic weibo posted by Lexing Shanghai')
    for index, row in official_traffic_data.iterrows():
        print('Coping with the {}th weibo'.format(index))
    ...
    print('Done!')
    print('Saving the geocode result...')
    np.save(os.path.join(weibo_data_path, 'geocode_traffic_account.npy'), geocode_result)
...
if __name__ == '__main__':
    ...
    # geocode_offical(gmap_client=gmaps)
    result = geocoding_string(gmap_client=gmaps, string='我在上海延安高架，太堵了')
    print(result)
